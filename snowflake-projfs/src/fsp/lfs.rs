...
use windows::Win32::Foundation::{HANDLE, INVALID_HANDLE_VALUE, NTSTATUS, STATUS_INSUFFICIENT_RESOURCES, STATUS_PENDING, STATUS_SUCCESS};
...
pub fn lfs_read_file(handle: HANDLE, buffer: &mut [u8], offset: u64, out: &mut usize) -> NTSTATUS {
    ...
    let event = SafeDropHandle::from(event.unwrap());
    let mut iosb: MaybeUninit<IO_STATUS_BLOCK> = MaybeUninit::uninit();
